...

{
	...
	CameraBoom->SocketOffset = FVector(0.f, 35.f, 75.f);
...
void AShooterCharacter::BeginPlay()
{
	...
	// Create FInterpLocations structs for each interp location.  Add to array
...
bool AShooterCharacter::GetBeamEndLocation(const FVector& MuzzleSocketLocation, FHitResult& OutHitResult)
{
	...
	if (bCrosshairHit)
	{
		// Tenative beam location - still need to trace from gun barrel
	...
	if (!OutHitResult.bBlockingHit) // object between weapon barrel and BeamEndPoint?
	{
			...
			if (BeamHitResult.GetActor())
			{
				...
				if (HitEnemy)
				{
					...
					if (BeamHitResult.BoneName.ToString() == HitEnemy->GetHeadBone())
					{
						...
						float Multiplier = EquippedWeapon->GetDamageRarityMultiplier();
						int32 FinalDamage = Damage + (Damage * Multiplier);
					...
					else
					{
						...
						UGameplayStatics::ApplyDamage(
							...
							this,
							UDamageType::StaticClass());
						HitEnemy->ShowHitNumber(FinalDamage, BeamHitResult.Location, false);
...
void AShooterCharacter::ReloadWeapon()
{
	...
	// Do we have ammo of the correct weapon type?
...
void AShooterCharacter::ExchangeInventoryItems(int32 CurrentItemIndex, int32 NewItemIndex)
{
	...
	if (bCanExchangeItems)
	{
		...
		OldEquippedWeapon->SetItemState(EItemState::EIS_Picked);
...
void AShooterCharacter::FinishReloading()
{
	...
	if (AmmoMap.Contains(AmmoType))
	{
		// Amout of ammo the Character is carrying of the EquipedWeapon type
...
FVector AShooterCharacter::GetCameraInterpLocation()
{
	...
	// Desired location = CameraWorldLocation + (Forward Vector * A) + (Up Vector * B)
